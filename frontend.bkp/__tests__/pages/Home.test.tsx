...
import HomeWrapper from '../../app/page';
...
describe('Home Page', () => {
    ...
    it('renders dashboard with cluster and deployment stats', async () => {
        ...
        // Check cluster count specifically within the total clusters section  
        const clusterStatsSection = screen.getByText(/total clusters/i).closest('div');
        expect(clusterStatsSection).toContainHTML('2');

        // Check deployment count specifically within the active deployments section
        const deploymentStatsSection = screen.getByText(/active deployments/i).closest('div');
        expect(deploymentStatsSection).toContainHTML('2');
    ...
    it('displays proper page title', () => {
        (useClusters as jest.Mock).mockReturnValue({
            clusters: [{ id: '1', name: 'test-cluster', provider: 'aws', region: 'us-west-2', status: 'active', nodes: 1 }],
            loading: false,
        });
        (useDeployments as jest.Mock).mockReturnValue({
            deployments: [],
    ...
    it('handles empty data state gracefully', () => {
        ...
        // When there are no clusters, the component shows a loading skeleton with animate-pulse
        expect(screen.getByTestId('sidebar')).toBeInTheDocument();
        // Should show loading skeleton for empty state
